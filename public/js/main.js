...
/*----------------------------------------*/
const TOTAL_RAM = 32 /*valor exacto de GB*/
...
function use_ram(ram) {
  ...
  ram_usage_uso.innerHTML = `${gbInUse_uso.toFixed(2)} GB`;
      ...
      let progress_ram = gbInUse_uso.toFixed(2);

      function updateProgress_ram(value) {
        const progressBar_ram= document.getElementById('ram_bar');
        progress_ram = value;
        if (progress_ram > TOTAL_RAM) {
          progress_ram = TOTAL_RAM;
        }
        progressBar_ram.style.width = (progress_ram/TOTAL_RAM*100)+ '%';
      }
